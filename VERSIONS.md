...
- **Multi-species substitution**: simultaneous binary substitution on up to two distinct crystallographic sites in a single run. Joint configurations enumerated under the full crystal symmetry. INSOD uses one `nsubs` line per target site (e.g. `2` on line 1, `1` on line 2).
...
- `example12` added: complex perovskite La₀.₇₅Sr₀.₂₅Mn₀.₂₅Fe₀.₇₅O₃ in a 2×2×2 supercell — multi-species example (binary substitution on two sites simultaneously); 13 inequivalent configurations from 784 total; GULP input files (FILER=1). Note: combined multi-species + multi-nary substitutions (ternary or higher per site within a multi-site enumeration) are not yet supported.
