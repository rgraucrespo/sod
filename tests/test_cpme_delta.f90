!*******************************************************************************
!    test_cpme_delta — correctness gate for the incremental swap evaluator.
!
!    Run inside a working directory that already holds a built CPME model
!    (INSOD, SGO, EQMATRIX + CPME training data — exactly the inputs mcsod
!    needs). Initialises the model the same way mcsod does, then checks the
!    incremental update cpme_evaluate_swap_delta against the full
!    cpme_evaluate_configuration in two ways:
!
...
program test_cpme_delta
  ...
  recal_ok = .true.
  have_energy = cpme_energies_available()
  if (have_energy) call cpme_preload_recalibration(target_level, recal_ok)

  npos_val = cpme_get_npos()
  if (.not. have_energy) then
    write(error_unit,'(A)') ' test_cpme_delta: model has no energies — cannot test delta evaluator.'
    stop 1
  end if
  if (npos_val < 2) then
    write(error_unit,'(A)') ' test_cpme_delta: need at least 2 sites.'
    stop 1
  end if

  write(output_unit,'(A,I0,A,I0)') ' test_cpme_delta: npos = ', npos_val, &
  ...
  if (n_fail == 0) then
    write(output_unit,'(A)') ' test_cpme_delta: PASS'
    call cpme_finalize_model()
    stop 0
  else
    write(error_unit,'(A,I0,A)') ' test_cpme_delta: FAIL (', n_fail, ' mismatches)'
    call cpme_finalize_model()
    stop 1
  end if
...
contains
  ...
  subroutine bench_scaling(npos_val)
    !  Per-call timing: full cpme_evaluate_configuration vs the incremental
    !  old_terms + cpme_evaluate_swap_delta, at several substitution levels.
    ...
    do il = 1, nlev
      ...
      do i = 1, m_calls
        ...
        call cpme_evaluate_configuration(newset, level, e, elo, ehi, lo, hi)
      ...
      call random_subset(npos_val, level, subset)
      call cpme_evaluate_configuration(subset, level, e, elo, ehi, lo, hi)
      call cpu_time(t0)
      do i = 1, m_calls
        call pick_swap(subset, level, npos_val, remove_idx, a, b)
        if (a < 0) cycle
        call cpme_evaluate_swap_delta(subset, level, a, b, dlo, dhi)
  ...
  subroutine check_level_equivalence(level, n_swaps, n_fail, max_term_err, max_energy_err)
    ...
    do i = 1, n_swaps
      ...
      call cpme_evaluate_configuration(subset, level, e_old, elo_old, ehi_old, low_old, high_old)

      ! incremental delta for the swap a -> b (build-path: routine rebuilds holes)
      call cpme_evaluate_swap_delta(subset, level, a, b, dlow, dhigh)
      ...
      call cpme_evaluate_swap_delta(subset, level, a, b, dlow2, dhigh2, holes, hc)

      ! apply swap and full-evaluate the new config
      newset = subset
      newset(remove_idx) = b
      call mc_insertion_sort(newset, level)
      call cpme_evaluate_configuration(newset, level, e_new, elo_new, ehi_new, low_new, high_new)
  ...
  subroutine check_drift(level, n_steps, n_fail, max_drift_err)
    ...
    call random_subset(npos_val, level, subset)
    call cpme_evaluate_configuration(subset, level, e0, elo0, ehi0, low_run, high_run)
    ...
    do i = 1, n_steps
      ...
      call cpme_evaluate_configuration(subset, level, e_full, elo_f, ehi_f, low_full, high_full)
