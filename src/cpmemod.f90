...
module cpmemod
  ...
  real(real64), save :: alpha_hybrid = 2.0_real64   ! power-law exponent for CPMEh weighting
  real(real64), save :: eta_hybrid   = 0.0_real64   ! log-scale asymmetry; >0 favours low-end
  integer, save :: cpme_choice = 2            ! 0=CPME0, 1=CPME1, 2=CPMEh (default)
  integer, save :: cpme_order_cap = max_motif_order  ! user-specified cap on expansion order
  integer, save :: cpme_target_level = 0            ! set during initialize; guards reference loading
  logical, save :: cpme_calibration_applied = .false.
  integer, save :: calib_config_list(9) = 0  ! config indices from INCPME calib_config_list
  ...
  ! Populated during V fitting (load_*_side_model) or loaded from INCPME V block.
  ...
  character(len=256), save :: cpme_model_filename = 'cpme.model'
  ...
  ! not be evaluated.
  logical, save :: cpme_energies_avail = .true.
  ...
  public :: cpme_initialize_model, cpme_finalize_model, cpme_get_target_level_from_insod
  public :: cpme_write_level_outputs, cpme_print_model_summary, cpme_evaluate_configuration
  public :: cpme_evaluate_swap_delta
  public :: cpme_get_npos, cpme_get_max_low_order, cpme_get_max_high_order
  public :: cpme_has_high_side
  public :: cpme_energies_available
  public :: cpme_preload_recalibration
  public :: cpme_get_target_atini, cpme_get_v0
  public :: cpme_get_epsilon
  public :: cpme_get_alpha_eta
  public :: cpme_get_choice
  ...
  public :: format_level_directory, write_cpme_model_template, write_cpme_model_calibrated
  public :: cpme_set_model_filename
  public :: cpme_variant_dir_from_model
  public :: cpme_get_newsymbol

contains

  subroutine cpme_initialize_model(target_level, energy_optional)
    !  Build the CPME effective Hamiltonian for the given target level.
    ...
    !  the V expansion is skipped and cpme_energies_avail is set .false., so
    ...
    cpme_target_level = target_level
    call read_eqmatrix_file('EQMATRIX')
    call build_target_layout_from_inputs(target_species)
    if (npos <= 0) then
      write(error_unit,'(A)') 'Error: invalid EQMATRIX header while initialising CPME Hamiltonian.'
      stop 1
    end if
    ...
    if (opt .and. .not. low_side_refs_present()) then
      cpme_energies_avail = .false.
      model_initialized  = .true.
      return
    end if
    cpme_energies_avail = .true.

    call read_cpme_model_v_block(trim(cpme_model_filename), v_ok)
    ...
    if (v_loaded_from_cache) then
      write(*,'(A,A,A)') ' > V parameters loaded from ', trim(cpme_model_filename), &
    ...
    model_initialized = .true.
  end subroutine cpme_initialize_model

  subroutine cpme_finalize_model()
    ...
    cpme_choice = 2
    cpme_order_cap = max_motif_order
    cpme_target_level = 0
    n_calib_used   = 0
    rms_calib_low  = 0.0_real64
    rms_calib_high = 0.0_real64
  end subroutine cpme_finalize_model

  subroutine cpme_set_model_filename(filename)
    character(len=*), intent(in) :: filename
    cpme_model_filename = trim(filename)
  end subroutine cpme_set_model_filename

  integer function cpme_get_npos() result(value)
    value = npos
  end function cpme_get_npos

  integer function cpme_get_max_low_order() result(value)
    value = max_low_order
  end function cpme_get_max_low_order

  integer function cpme_get_max_high_order() result(value)
    value = max_high_order
  end function cpme_get_max_high_order

  logical function cpme_is_initialized() result(value)
    value = model_initialized
  end function cpme_is_initialized

  logical function cpme_has_high_side() result(value)
    value = high_base_loaded
  end function cpme_has_high_side

  logical function cpme_energies_available() result(value)
    ...
    value = cpme_energies_avail
  end function cpme_energies_available
  ...
  integer function cpme_get_choice() result(value)
    value = cpme_choice
  end function cpme_get_choice

  integer function cpme_get_target_species() result(value)
    value = target_species
  end function cpme_get_target_species

  integer function cpme_get_nop() result(value)
    value = nop
  end function cpme_get_nop

  integer function cpme_get_target_atini() result(value)
    value = target_atini
  end function cpme_get_target_atini

  subroutine cpme_get_v0(v0_low_out, v0_high_out)
    real(real64), intent(out) :: v0_low_out, v0_high_out
    v0_low_out  = v0_low
    v0_high_out = v0_high
  end subroutine cpme_get_v0

  subroutine cpme_get_newsymbol(orig_sym_out, sym_new_out, sym_rem_out)
  ...
  end subroutine cpme_get_newsymbol

  subroutine cpme_get_epsilon(eps_low_out, eps_high_out)
    !  Return the per-order renormalisation parameters epsilon (CPME §3.2).
    real(real64), intent(out) :: eps_low_out(0:4), eps_high_out(0:4)
    eps_low_out  = eps_low(0:4)
    eps_high_out = eps_high(0:4)
  end subroutine cpme_get_epsilon

  subroutine cpme_get_alpha_eta(alpha_out, eta_out)
    real(real64), intent(out) :: alpha_out, eta_out
    alpha_out = alpha_hybrid
    eta_out   = eta_hybrid
  end subroutine cpme_get_alpha_eta

  subroutine cpme_print_model_summary()
    integer :: k
    ...
    write(*,*)
    if (.not. cpme_energies_avail) then
      write(*,'(A)') '    Geometry-only: no CPME Hamiltonian built (uniform sampling'
    ...
    if (v_loaded_from_cache) then
      write(*,'(A,A,A)') '      V parameters: loaded from ', trim(cpme_model_filename), ' cache'
    else
    ...
    write(*,*)
  end subroutine cpme_print_model_summary

  subroutine cpme_get_blend_weights(level, weight_low, weight_high)
    integer, intent(in) :: level
    real(real64), intent(out) :: weight_low, weight_high

    call require_model_initialized('cpme_get_blend_weights')
    call validate_level(level, 'cpme_get_blend_weights')
  ...
  end subroutine cpme_get_blend_weights

  subroutine cpme_load_level_ensemble(level, mm, conf, omega, ok)
    integer, intent(in) :: level
    ...
    call require_model_initialized('cpme_load_level_ensemble')
    call validate_level(level, 'cpme_load_level_ensemble')
  ...
  end subroutine cpme_load_level_ensemble

  subroutine cpme_load_level_dataset(level, mm, conf, omega, energies, ok)
    integer, intent(in) :: level
    integer, intent(out) :: mm
    integer, allocatable, intent(out) :: conf(:, :), omega(:)
    ...
    call require_model_initialized('cpme_load_level_dataset')
    call validate_level(level, 'cpme_load_level_dataset')
    call read_level_data(level, mm, conf, omega, energies, ok)
  end subroutine cpme_load_level_dataset

  subroutine cpme_build_ordered_configuration(level, conf, ok)
    integer, intent(in) :: level
    integer, allocatable, intent(out) :: conf(:)
    logical, intent(out) :: ok
    integer :: i

    call require_model_initialized('cpme_build_ordered_configuration')
    ok = .false.
    ...
    ok = .true.
  end subroutine cpme_build_ordered_configuration

  logical function cpme_configuration_is_valid(conf_row) result(ok)
    ...
    ok = .true.
  end function cpme_configuration_is_valid

  subroutine cpme_sort_configuration(conf_row)
  ...
  end subroutine cpme_sort_configuration

  subroutine cpme_evaluate_subset(conf_row, energy, energy_low, energy_high, low_terms, high_terms)
    integer, intent(in) :: conf_row(:)
    ...
    call require_model_initialized('cpme_evaluate_subset')
    if (.not. cpme_configuration_is_valid(conf_row)) then
      write(error_unit,'(A)') 'Error: invalid configuration passed to cpme_evaluate_subset.'
      stop 1
    end if

    call cpme_evaluate_configuration(conf_row, size(conf_row), energy, local_energy_low, local_energy_high, &
  ...
  end subroutine cpme_evaluate_subset

  subroutine cpme_get_target_level_from_insod(target_level)
    integer, intent(out) :: target_level
    type(insod_t) :: d
    call read_insod('INSOD', d)
    target_level = d%nsubs_max
    if (d%nsubs_min /= d%nsubs_max) &
      write(*,*) ' > INSOD range detected; using upper endpoint as CPME target level:', target_level
  end subroutine cpme_get_target_level_from_insod
  ...
  subroutine build_target_layout_from_inputs(species_index)
    ...
    ! Populate module-level cache for write_cpme_model_template
  ...
  subroutine cpme_write_level_outputs(target_level)
    integer, intent(in) :: target_level
    integer :: mm, m, choice
    ...
    call require_model_initialized('cpme_write_level_outputs')
    call validate_level(target_level, 'cpme_write_level_outputs')
    call format_level_directory(target_level, level_dir)
    call cpme_load_level_ensemble(target_level, mm, conf, omega, ok)
    if (.not. ok) then
    ...
    do m = 1, mm
      if (target_level > 0) then
        call cpme_evaluate_configuration(conf_by_level(1:target_level, m), target_level, &
    ...
    model_tmp_path = trim(cpme_model_filename)//'.tmp'
    level_model_tmp_path = trim(level_dir)//'/'//trim(cpme_model_filename)//'.tmp'
    call cpme_preload_recalibration(target_level, ok)
    model_path = trim(cpme_model_filename)
    inquire(file=trim(model_path), exist=have_model)

    if (.not. have_model .and. target_level > 0 .and. mm > 0) then
      call write_cpme_model_template(model_tmp_path, all_low_terms, all_high_terms, mm, target_level)
      ...
      write(*,'(A,A,A)') '   add energies to nXX/ENERGIES, rename ', trim(model_tmp_path), &
        ' to '//trim(cpme_model_filename)//', re-run cpmesod.'
    end if

    if (have_model .and. cpme_calibration_applied) then
      call write_cpme_model_calibrated(model_tmp_path)
      write(*,'(A,A)') ' > Calibrated model written to ', trim(model_tmp_path)
      write(*,'(A,A,A)') '   Rename ', trim(model_tmp_path), ' to '//trim(cpme_model_filename)//' to use on future runs.'
    end if

    if (have_model) then
      call write_cpme_variant_outputs(cpme_choice, target_level, level_dir, &
        all_low_terms, all_high_terms, mm)
    else
      if (high_base_loaded) then
        do choice = 0, 2
          call write_cpme_variant_outputs(choice, target_level, level_dir, &
            all_low_terms, all_high_terms, mm)
        end do
      else
        call write_cpme_variant_outputs(0, target_level, level_dir, &
          all_low_terms, all_high_terms, mm)
      end if
    end if
  ...
  end subroutine cpme_write_level_outputs

  subroutine write_cpme_variant_outputs(choice, target_level, level_dir, all_low_terms, all_high_terms, mm)
    ...
    call cpme_variant_directory(choice, variant_dir)
    output_dir = trim(level_dir)//'/'//trim(variant_dir)
    call execute_command_line('mkdir -p '//trim(output_dir), exitstat=ios_exec)
    if (ios_exec /= 0) then
      write(error_unit,'(A,A)') 'Error: could not create CPME output directory ', trim(output_dir)
      stop 1
    end if
  ...
  end subroutine write_cpme_variant_outputs

  subroutine compute_cpme_variant_energy(choice, level_val, low_terms, high_terms, energy, e_low_cal, e_high_cal)
    ...
    eff_low_ord  = min(max_low_order,  cpme_order_cap)
    eff_high_ord = min(max_high_order, cpme_order_cap)
  ...
  end subroutine compute_cpme_variant_energy

  subroutine cpme_variant_directory(choice, dirname)
  ...
  end subroutine cpme_variant_directory

  subroutine cpme_variant_dir_from_model(filename, dirname, ok)
    !  Read just the CPME choice (first data line) from a cpme.model file and
    !  return the corresponding variant directory name ('CPME0'/'CPME1'/'CPMEh').
    ...
    !  missing or the choice line is unreadable (dirname defaults to 'CPMEh').
    character(len=*), intent(in)  :: filename
    character(len=*), intent(out) :: dirname
    logical,          intent(out) :: ok
    ...
    call cpme_variant_directory(choice, dirname)
    ok = .true.
  end subroutine cpme_variant_dir_from_model

  subroutine cpme_evaluate_configuration(conf_row, level, energy, energy_low, energy_high, low_terms, high_terms)
    integer, intent(in) :: conf_row(:)
    integer, intent(in) :: level
  ...
  end subroutine cpme_evaluate_configuration

  subroutine cpme_evaluate_swap_delta(conf_old, level, removed_site, added_site, &
    ...
    !  cheaper per order than the full cpme_evaluate_configuration sum.
  ...
  end subroutine cpme_evaluate_swap_delta
  ...
  subroutine load_low_side_model()
    ...
    if (cpme_target_level > 0) low_max_level = min(max_motif_order, cpme_target_level - 1)
  ...
  subroutine load_high_side_model()
    ...
    do hole_count = 1, min(max_motif_order, npos)
      ...
      if (cpme_target_level > 0 .and. ge_count <= cpme_target_level) exit
  ...
  subroutine require_model_initialized(context)
    ...
    if (.not. model_initialized) then
      write(error_unit,'(A,1X,A)') 'Error: CPME Hamiltonian is not initialized in', trim(context)
      stop 1
    end if
  ...
  subroutine level_blend_weights(level, total_sites, weight_low, weight_high)
    ...
    !  with full weight, where xref = cpme_order_cap / total_sites.
    !
    ...
    xref = real(cpme_order_cap, real64) / real(total_sites, real64)
  ...
  subroutine read_calib_data(target_level, calib_list, n_list, n_use, &
    ...
    integer,      intent(in)  :: target_level
    integer,      intent(in)  :: calib_list(:)   ! config indices (1-based) from INCPME
    ...
    call cpme_load_level_ensemble(target_level, mm_all, conf_all, omega_all, read_ok)
    ...
    do i = 1, n_use
      ...
      if (target_level > 0) then
        call cpme_evaluate_configuration(conf_all(idx, 1:target_level), target_level, &
          energy, energy_low, energy_high, low_terms, high_terms)
      else
        low_terms  = 0.0_real64
        high_terms = 0.0_real64
  ...
  subroutine write_cpme_model_template(filename, all_low_terms, all_high_terms, mm, target_level)
    !  Writes SODPROJECT/INCPME.tmp with the new 7-data-line format:
    !    CPME choice, CPMEorder, n_calib, calib_config_list, eps_low, eps_high, alpha eta.
    ...
    do idx = 1, mm
      call compute_cpme_variant_energy(cpme_choice, target_level, all_low_terms(:, idx), all_high_terms(:, idx), &
    ...
    variant_choice = cpme_choice
    if (.not. high_base_loaded) variant_choice = 0
    order_cap_out = min(cpme_order_cap, max(max_low_order, max_high_order))
    ...
    write(unit_tmp,'(A)') '# SOD CPME calibration/control file.'
    write(unit_tmp,'(A,A,A)') '# Rename this file to ', trim(cpme_model_filename), ' to enable calibration.'
    write(unit_tmp,'(A,A,A)') '# Target level directory: ', trim(level_dir), '/'
    write(unit_tmp,'(A)') ''
    write(unit_tmp,'(A)') '# CPME Hamiltonian: 0 (CPME0 low-side); 1 (CPME1 high-side); 2 (CPMEh hybrid)'
    write(unit_tmp,'(I0)') variant_choice
    write(unit_tmp,'(A)') ''
    write(unit_tmp,'(A)') '# CPMEorder cap (2, 3, or 4)'
    ...
    write(unit_tmp,'(A)') ''
    write(unit_tmp,'(A)') '# epsilon_low (epsilon_0 ... epsilon_CPMEorder)'
    write(unit_tmp,'(*(1X,F10.6))') 0.0_real64, eps_low(1:order_cap_out)
    write(unit_tmp,'(A)') ''
    write(unit_tmp,'(A)') '# epsilon_high (epsilon_N ... epsilon_{N-CPMEorder})'
    write(unit_tmp,'(*(1X,F10.6))') 0.0_real64, eps_high(1:order_cap_out)
    write(unit_tmp,'(A)') ''
    write(unit_tmp,'(A)') '# CPMEh_alpha (>1.0 for smooth blending, default 2.0) and CPMEh_eta (default 0.0; >0 favours low-end, <0 favours high-end)'
    write(unit_tmp,'(F10.6,1X,F10.6)') 2.0_real64, 0.0_real64
    write(unit_tmp,'(A)') ''
    call write_v_block_section(unit_tmp, variant_choice)
    close(unit_tmp)
  end subroutine write_cpme_model_template

  ! Writes INCPME.tmp with n_calib=0 and the fitted eps/alpha/eta from module state,
  ! plus the V block.  Called automatically after a successful calibration run.
  subroutine write_cpme_model_calibrated(filename)
    character(len=*), intent(in) :: filename
    integer :: unit_tmp, eff_low_ord, eff_high_ord
    eff_low_ord  = min(max_low_order,  cpme_order_cap)
    eff_high_ord = min(max_high_order, cpme_order_cap)
    open(newunit=unit_tmp, file=trim(filename), status='replace', action='write')
    write(unit_tmp,'(A)') '# SOD CPME calibration/control file (calibrated — n_calib=0).'
    write(unit_tmp,'(A,A,A)') '# Rename to ', trim(cpme_model_filename), ' to use these parameters directly.'
    write(unit_tmp,'(A)') ''
    write(unit_tmp,'(A)') '# CPME Hamiltonian: 0 (CPME0 low-side); 1 (CPME1 high-side); 2 (CPMEh hybrid)'
    write(unit_tmp,'(I0)') cpme_choice
    write(unit_tmp,'(A)') ''
    write(unit_tmp,'(A)') '# CPMEorder cap (2, 3, or 4)'
    write(unit_tmp,'(I0)') cpme_order_cap
    write(unit_tmp,'(A)') ''
    write(unit_tmp,'(A)') '# n_calib: 0 no calibration (manual epsilons); 1..9 number of configurations for calibration'
    write(unit_tmp,'(I0)') 0
    write(unit_tmp,'(A)') ''
    ...
    write(unit_tmp,'(A)') ''
    call write_v_block_section(unit_tmp, cpme_choice)
    close(unit_tmp)
  end subroutine write_cpme_model_calibrated

  ! Writes the "# CPME energy terms" V-block section to an already-open unit.
  ! Uses the compact V residual arrays currently in module state.
  ! 3 columns (level m v) for CPME0/CPME1; 4 columns (level m vl vh) for CPMEh.
  subroutine write_v_block_section(unit_out, cpme_ch)
    integer, intent(in) :: unit_out, cpme_ch
    integer :: m

    if (nv1_low < 1 .and. nv1_high < 1) return

    write(unit_out,'(A)') '# CPME energy terms'
    write(unit_out,'(A)') ''
    if (cpme_ch == 2) then
      write(unit_out,'(I2,1X,I2,2(1X,F22.14))') 0, 1, v0_low, v0_high
    else if (cpme_ch == 0) then
    ...
    do m = 1, max(nv1_low, nv1_high)
      ...
      else if (cpme_ch == 0 .and. m <= nv1_low) then
        write(unit_out,'(I2,1X,I2,1X,F22.14)') 1, m, v1res_low(m)
      else if (cpme_ch == 1 .and. m <= nv1_high) then
    ...
    do m = 1, max(nv2_low, nv2_high)
      ...
      else if (cpme_ch == 0 .and. m <= nv2_low) then
        write(unit_out,'(I2,1X,I2,1X,F22.14)') 2, m, v2res_low(m)
      else if (cpme_ch == 1 .and. m <= nv2_high) then
    ...
    do m = 1, max(nv3_low, nv3_high)
      ...
      else if (cpme_ch == 0 .and. m <= nv3_low) then
        write(unit_out,'(I2,1X,I2,1X,F22.14)') 3, m, v3res_low(m)
      else if (cpme_ch == 1 .and. m <= nv3_high) then
    ...
    do m = 1, max(nv4_low, nv4_high)
      ...
      else if (cpme_ch == 0 .and. m <= nv4_low) then
        write(unit_out,'(I2,1X,I2,1X,F22.14)') 4, m, v4res_low(m)
      else if (cpme_ch == 1 .and. m <= nv4_high) then
        write(unit_out,'(I2,1X,I2,1X,F22.14)') 4, m, v4res_high(m)
      end if
    end do
  end subroutine write_v_block_section

  subroutine read_cpme_model(filename, cpme_ch, order_cap, n_calib, &
      calib_list_out, n_calib_list_out, alpha_out, eta_out, eps_low_out, eps_high_out, ok)
    !  INCPME format (7 data lines; comment lines starting with '#' and blank lines are skipped):
    !    data line 1: CPME choice          (integer: 0=CPME0, 1=CPME1, 2=CPMEh)
    !    data line 2: CPMEorder cap        (integer: 2, 3, or 4; other values are a fatal error)
    ...
    !    data line 7: CPMEh_alpha CPMEh_eta  (two reals on one line; applied only for CPMEh)
    character(len=*), intent(in) :: filename
    integer,      intent(out) :: cpme_ch, order_cap, n_calib
    ...
    read(line, *, iostat=ios) cpme_ch
    if (ios /= 0) then; close(unit_in); return; end if
    if (cpme_ch < 0 .or. cpme_ch > 2) then; close(unit_in); return; end if

    ! Line 2: CPMEorder cap (must be 2, 3, or 4)
    call next_data_line(unit_in, line, lk)
    if (.not. lk) then; close(unit_in); return; end if
    read(line, *, iostat=ios) order_cap
    if (ios /= 0) then; close(unit_in); return; end if
    if (order_cap < 2 .or. order_cap > 4) then
      write(*,'(A,A,A,I0,A)') ' > Error: ', trim(cpme_model_filename), ' CPMEorder = ', order_cap, '; must be 2, 3, or 4.'
      stop 1
    end if
    ...
    if (n_calib < 0 .or. n_calib > 9) then
      write(*,'(A,A,A,I0,A)') ' > Error: ', trim(cpme_model_filename), ' n_calib = ', n_calib, '; must be 0..9.'
      stop 1
    end if
    ...
    ! Line 7: CPMEh_alpha and CPMEh_eta (two reals on one line)
    call next_data_line(unit_in, line, lk)
    if (.not. lk) then; close(unit_in); return; end if
    read(line, *, iostat=ios) alpha_out, eta_out
    if (ios /= 0) then; close(unit_in); return; end if
    if (alpha_out <= 0.0_real64) then
      write(*,'(A,F10.4,A)') ' > Error: CPMEh_alpha = ', alpha_out, ' must be positive.'
      stop 1
    end if
    if (alpha_out <= 1.0_real64) then
      write(*,'(A,F10.4,A)') ' > Warning: CPMEh_alpha = ', alpha_out, &
        '; alpha > 1 is required for continuous first derivatives at the blending boundaries.'
    end if

    close(unit_in)
    ok = .true.
  end subroutine read_cpme_model

  ! Reads the optional V-block section from INCPME (lines after the 7 mandatory lines).
  ! Populates module-level compact V residual arrays and v0_low / v0_high.
  ! Format: one line per reference config — "level m v_low v_high" (CPMEh) or
  !         "level m v_side" (CPME0 or CPME1, 3 columns).  cpme_choice on line 1
  !         determines the column count.
  subroutine read_cpme_model_v_block(filename, ok)
    character(len=*), intent(in) :: filename
    logical, intent(out) :: ok

    integer :: unit_in, ios, i, lev, idx, cpme_ch_local
    character(len=512) :: line
    logical :: lk
    real(real64) :: vl, vh
    ...
    ! Read line 1 to get cpme_choice; skip lines 2–7
    call next_data_line(unit_in, line, lk)
    if (.not. lk) then; close(unit_in); return; end if
    read(line, *, iostat=ios) cpme_ch_local
    if (ios /= 0 .or. cpme_ch_local < 0 .or. cpme_ch_local > 2) then
      close(unit_in)
      deallocate(tmp1, tmp2, tmp3, tmp4, tmp1h, tmp2h, tmp3h, tmp4h)
      return
    end if
    ...
    do
      ...
      if (cpme_ch_local == 2) then
        read(line, *, iostat=ios) lev, idx, vl, vh
        if (ios /= 0) cycle
      else
        read(line, *, iostat=ios) lev, idx, vl
        if (ios /= 0) cycle
        if (cpme_ch_local == 1) then
          vh = vl;  vl = 0.0_real64
        end if
      end if
      if (lev < 0 .or. lev > 4 .or. idx < 1 .or. idx > max_m) cycle
      if (lev == 0 .and. idx == 1) then
        if (cpme_ch_local == 0 .or. cpme_ch_local == 2) v0_low  = vl
        if (cpme_ch_local == 1 .or. cpme_ch_local == 2) v0_high = vh
      else if (lev == 1) then
        if (cpme_ch_local == 0 .or. cpme_ch_local == 2) then; tmp1(idx)  = vl;  n1l = max(n1l, idx);  end if
        if (cpme_ch_local == 1 .or. cpme_ch_local == 2) then; tmp1h(idx) = vh;  n1h = max(n1h, idx);  end if
      else if (lev == 2) then
        if (cpme_ch_local == 0 .or. cpme_ch_local == 2) then; tmp2(idx)  = vl;  n2l = max(n2l, idx);  end if
        if (cpme_ch_local == 1 .or. cpme_ch_local == 2) then; tmp2h(idx) = vh;  n2h = max(n2h, idx);  end if
      else if (lev == 3) then
        if (cpme_ch_local == 0 .or. cpme_ch_local == 2) then; tmp3(idx)  = vl;  n3l = max(n3l, idx);  end if
        if (cpme_ch_local == 1 .or. cpme_ch_local == 2) then; tmp3h(idx) = vh;  n3h = max(n3h, idx);  end if
      else if (lev == 4) then
        if (cpme_ch_local == 0 .or. cpme_ch_local == 2) then; tmp4(idx)  = vl;  n4l = max(n4l, idx);  end if
        if (cpme_ch_local == 1 .or. cpme_ch_local == 2) then; tmp4h(idx) = vh;  n4h = max(n4h, idx);  end if
      end if
    end do
    close(unit_in)
    ...
    deallocate(tmp1, tmp2, tmp3, tmp4, tmp1h, tmp2h, tmp3h, tmp4h)
  end subroutine read_cpme_model_v_block
  ...
  ! ---------------------------------------------------------------------------
  ! FILER / structure-file helpers for INCPME.tmp embedding
  ! ---------------------------------------------------------------------------
  ...
  !  raw cpme_evaluate_configuration with symmetric power-law blending.
  ! ---------------------------------------------------------------------------
  ...
  subroutine check_target_vs_reference(target_level, cpme_ch, eff_low_ord, eff_high_ord)
    integer, intent(in) :: target_level, cpme_ch, eff_low_ord, eff_high_ord
    logical :: low_conflict, high_conflict
    low_conflict  = (cpme_ch == 0 .or. cpme_ch == 2) .and. (target_level <= eff_low_ord)
    high_conflict = (cpme_ch == 1 .or. cpme_ch == 2) .and. high_base_loaded .and. &
    ...
    if (low_conflict) then
      write(error_unit,'(A,I0,A,I0,A)') &
        ...
        '). CPME cannot predict its own training data.'
      stop 1
    end if
    if (high_conflict) then
      write(error_unit,'(A,I0,A,I0,A,I0,A)') &
        ...
        '..n', npos, '). CPME cannot predict its own training data.'
      stop 1
    end if
  ...
  subroutine note_target_energies_if_present(tgt)
    ...
    if (ex) write(*,'(A,A,A)') &
      ...
      '); CPME predictions can be benchmarked against them.'
  end subroutine note_target_energies_if_present

  ! ---------------------------------------------------------------------------
  !  Pre-load recalibration from SODPROJECT/INCPME and nXX/ENERGIES.
  !  Sets module vars: cpme_choice, cpme_order_cap, eps_low, eps_high, alpha_hybrid, eta_hybrid.
  !  Must be called BEFORE the MC walk or energy output so that Metropolis
  !  acceptance uses calibrated Hamiltonian energies.
  !  If INCPME is absent, sensible defaults apply (CPMEh if high side available,
  !  CPME0 otherwise; eps=1, alpha=2, eta=0).
  ! ---------------------------------------------------------------------------

  subroutine cpme_preload_recalibration(target_level, ok)
    integer, intent(in)  :: target_level
    logical, intent(out) :: ok
    ...
    integer  :: cpme_ch, order_cap, n_calib_req, eff_low_ord, eff_high_ord, n_use, n_calib_read
    ...
    real(real64) :: rms_before_low, rms_before_high, rms_before_cpmeh
    real(real64) :: rms_loo_low, rms_loo_high, rms_after_cpmeh
    ...
    logical :: low_fit_ok, high_fit_ok, use_loo_low, use_loo_high, use_loo_cpmeh
    ...
    cpme_choice    = 2
    cpme_order_cap = max_motif_order
    cpme_calibration_applied = .false.

    call format_level_directory(target_level, level_dir)
    model_path = trim(cpme_model_filename)
    model_snapshot_path = trim(level_dir)//'/'//trim(cpme_model_filename)
    inquire(file=trim(model_path), exist=have_model)

    if (.not. have_model) then
      if (high_base_loaded) then
        cpme_choice = 2
        write(*,'(A,A,A)') ' > No ', trim(cpme_model_filename), ' found; using CPMEh with eps=1, alpha=2, eta=0.'
      else
        cpme_choice = 0
        write(*,'(A,A,A)') ' > No ', trim(cpme_model_filename), ' found, no high-side data: using CPME0 with eps=1.'
      end if
      eff_low_ord   = min(max_low_order,  cpme_order_cap)
      eff_high_ord  = min(max_high_order, cpme_order_cap)
      write(*,'(A,I0,A,I0,A)') ' > Effective order applied: low=', eff_low_ord, ', high=', eff_high_ord, '.'
      call check_target_vs_reference(target_level, cpme_choice, eff_low_ord, eff_high_ord)
      return
    end if
    ...
    call read_cpme_model(model_path, cpme_ch, order_cap, n_calib_req, &
      calib_list_read, n_calib_list_read, alpha_read, eta_read, eps_low_read, eps_high_read, ok)
    if (.not. ok) then
      write(*,'(A,A,A)') ' > Warning: could not parse ', trim(cpme_model_filename), '; using defaults (CPMEh, eps=1, alpha=2, eta=0).'
      ok = .true.
      return
    end if
    ...
    cpme_choice    = cpme_ch
    cpme_order_cap = order_cap
    eff_low_ord   = min(max_low_order,  cpme_order_cap)
    eff_high_ord  = min(max_high_order, cpme_order_cap)

    if (eff_low_ord < cpme_order_cap .and. (cpme_ch == 0 .or. cpme_ch == 2)) then
      write(*,'(A,I0,A,I0,A)') ' > Warning: CPMEorder cap ', cpme_order_cap, &
        ' exceeds available low-side orders ', eff_low_ord, '; capping.'
    end if
    if (high_base_loaded .and. eff_high_ord < cpme_order_cap .and. (cpme_ch == 1 .or. cpme_ch == 2)) then
      write(*,'(A,I0,A,I0,A)') ' > Warning: CPMEorder cap ', cpme_order_cap, &
    ...
    if (n_calib_req == 0) then
      if (cpme_ch == 0 .or. cpme_ch == 2) eps_low(0:eff_low_ord)  = eps_low_read(0:eff_low_ord)
      if (cpme_ch == 1 .or. cpme_ch == 2) then
      ...
      cpme_calibration_applied = .true.
      write(*,'(A,A,A)', advance='no') ' > eps read from ', trim(cpme_model_filename), ' (n_calib=0).'
      if (cpme_ch == 0 .or. cpme_ch == 2) then
        write(*,'(A)', advance='no') '  ε_low ='
        do iref = 0, eff_low_ord
          write(*,'(1X,F9.5)', advance='no') eps_low(iref)
        end do
      end if
      if ((cpme_ch == 1 .or. cpme_ch == 2) .and. high_base_loaded) then
    ...
    cpme_calibration_applied = .true.   ! cpme.model was read; calibration attempted
    n_use = n_calib_read
    n_calib_used = n_use
    ...
    rms_before_cpmeh = sqrt(resid_sum / real(n_use, real64))
    ...
    ! --- Low-side ---
    use_loo_low = .false.
    if ((cpme_ch == 0 .or. cpme_ch == 2) .and. eff_low_ord >= 1) then
      if (n_use <= eff_low_ord) then
        ! Underdetermined: keep upper eps from INCPME, fit lower n_use epsilons (eps_0..eps_{n_use-1})
    ...
    ! --- High-side ---
    use_loo_high = .false.
    if (high_base_loaded .and. (cpme_ch == 1 .or. cpme_ch == 2) .and. eff_high_ord >= 1) then
    ...
    ! --- CPMEh combined score (after both sides fitted) ---
    if (cpme_ch == 2 .and. high_base_loaded .and. (low_fit_ok .or. high_fit_ok)) then
      ! LOO-CV for CPMEh: re-fit both sides per fold; requires both to be LOO-eligible
      use_loo_cpmeh = low_fit_ok .and. high_fit_ok .and. &
                     n_use >= max(eff_low_ord, eff_high_ord) + 2
      if (use_loo_cpmeh) then
        resid_sum = 0.0_real64
        do jloo = 1, n_use
          loo_n = 0
          do iref = 1, n_use
          ...
          call fit_epsilon(v0_low, calib_low_contribs, n_calib_read, eff_low_ord, &
            loo_idx(1:loo_n), calib_energies, loo_n, eps_loo, rms_tmp, fit_ok_loo)
          if (.not. fit_ok_loo) then; use_loo_cpmeh = .false.; exit; end if
          e_loo_low = v0_low + eps_loo(0) + &
            dot_product(eps_loo(1:eff_low_ord), calib_low_contribs(1:eff_low_ord, jloo))
          call fit_epsilon(v0_high, calib_high_contribs, n_calib_read, eff_high_ord, &
            loo_idx(1:loo_n), calib_energies, loo_n, eps_loo, rms_tmp, fit_ok_loo)
      ...
      if (use_loo_cpmeh) then
        rms_after_cpmeh = sqrt(resid_sum / real(n_use, real64))
        write(*,'(A,I0,A,F9.5,A,F9.5,A)') ' > CPMEh LOO-CV (', n_use, ' configs): ', &
          rms_before_cpmeh, ' -> ', rms_after_cpmeh, ' eV'
      else
        resid_sum = 0.0_real64
        do iref = 1, n_use
        ...
        rms_after_cpmeh = sqrt(resid_sum / real(n_use, real64))
        write(*,'(A,I0,A,F9.5,A,F9.5,A)') ' > CPMEh RMSE (', n_use, ' configs): ', &
          rms_before_cpmeh, ' -> ', rms_after_cpmeh, ' eV'
      end if
    end if

    ! --- Alpha/eta (CPMEh only, manual) ---
    if (cpme_ch == 2) then
      alpha_hybrid = alpha_read
      eta_hybrid   = eta_read
    end if
  ...
  end subroutine cpme_preload_recalibration
